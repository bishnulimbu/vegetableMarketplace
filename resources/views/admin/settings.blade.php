...
@section('content')
    <div class="flex items-center justify-between mb-8">
        <div>
            <h1 class="text-3xl font-bold text-slate-900">{{ __('Marketplace Settings') }}</h1>
            <p class="text-slate-500 mt-1">{{ __('Customize your marketplace name and description.') }}</p>
        </div>
    </div>
    ...
    <div class="max-w-2xl rounded-2xl bg-white border border-green-100 p-8">
        <div class="flex items-center gap-3 mb-6">
            ...
            <h2 class="text-lg font-semibold">{{ __('General Settings') }}</h2>
        </div>

        <form action="{{ route('admin.settings.update') }}" method="POST" class="space-y-5">
            @csrf
            <label class="block text-sm font-medium text-slate-700">
                <span>{{ __('Market Name') }}</span>
            ...
            <label class="block text-sm font-medium text-slate-700">
                <span>{{ __('Market Description') }}</span>
