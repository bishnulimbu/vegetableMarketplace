...
@section('content')
    ...
    <form method="GET" action="{{ route('consumer.market') }}" class="mb-8 space-y-4">
        <div class="flex flex-col sm:flex-row gap-3">
            ...
            <div class="relative flex-1">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search vegetables..." class="w-full rounded-xl border border-slate-200 pl-11 pr-4 py-3 focus:border-market-400 focus:ring-2 focus:ring-market-100 outline-none transition" />
            </div>
            {{-- Condition filter --}}
            ...
            {{-- Vendor filter --}}
            <input type="text" name="vendor" value="{{ request('vendor') }}" placeholder="Vendor name..." class="rounded-xl border border-slate-200 px-4 py-3 focus:border-market-400 focus:ring-2 focus:ring-market-100 outline-none transition min-w-[140px]" />
            {{-- Submit / Reset --}}
            <button type="submit" class="rounded-full veg-gradient px-6 py-3 text-white font-medium hover:opacity-90 transition">Filter</button>
            @if(request()->anyFilled(['search', 'condition', 'vendor']))
                <a href="{{ route('consumer.market') }}" class="rounded-full border border-slate-200 px-6 py-3 text-slate-600 font-medium hover:bg-slate-50 transition text-center">Clear</a>
            @endif
        </div>
    </form>
    ...
    @if($vegetables->isEmpty())
        <div class="rounded-2xl border-2 border-dashed border-green-200 p-16 text-center">
            ...
            <h2 class="text-xl font-semibold text-slate-700 mb-2">No matching vegetables</h2>
        ...
        <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            @foreach($vegetables as $vegetable)
                <a href="{{ route('product.view', $vegetable) }}" class="block rounded-2xl bg-white border border-green-100 overflow-hidden card-hover">
                    ...
                    <div class="p-5">
                        <h2 class="font-semibold text-lg text-slate-900 hover:text-market-600 transition">{{ $vegetable->name }}</h2>
