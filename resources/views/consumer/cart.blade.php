...
@section('content')
    <div class="flex items-center justify-between mb-8">
        <div>
            ...
            <p class="text-slate-500 mt-1">{{ $cartItems->count() }} item(s) in your cart</p>
        </div>
        <a href="{{ route('consumer.market') }}" class="text-sm text-market-600 hover:text-market-700 font-medium">← Continue shopping</a>
    </div>
    ...
    @if($cartItems->isEmpty())
        <div class="rounded-2xl border-2 border-dashed border-green-200 p-16 text-center">
            ...
            <p class="text-slate-500 mb-5">Browse the market and add items to your cart.</p>
                ...
                @foreach($cartItems as $item)
                    <div class="rounded-2xl bg-white border border-green-100 p-4 flex gap-4 items-center">
                        ...
                        <form action="{{ route('cart.update', $item) }}" method="POST" class="flex items-center gap-2">
                            ...
                            <button type="submit" class="text-xs text-market-600 hover:text-market-700 font-medium">Update</button>
                        </form>
            ...
            <div class="rounded-2xl bg-white border border-green-100 p-6 h-fit">
                ...
                <div class="space-y-3 text-sm">
                    ...
                    <div class="flex justify-between">
                        ...
                        <span class="font-medium">{{ $delivery === 0 ? 'Free' : 'Rs. ' . number_format($delivery, 2) }}</span>
                    </div>
                    <div class="border-t border-green-100 pt-3 flex justify-between text-lg">
                        <span class="font-semibold">Total</span>
                        <span class="font-bold text-market-600">Rs. {{ number_format($subtotal + $delivery, 2) }}</span>
                    </div>
                </div>
